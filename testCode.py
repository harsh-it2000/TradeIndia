...
#load chrome driver 
driver = webdriver.Chrome(executable_path="C:\\Users\\harsh\\Downloads\\chromedriver_win32\\chromedriver.exe")
...
while True:
    # if(current_time >= "09:30:00" and current_time <= "15:00:00"):
        ...
        if ( RSI >= 30 and last_order=="sell" and MACD >= -3 and EMA == "BUY" and RSI <=70):
            ...
            time.sleep(2)
            driver.find_element(By_XPATH, "//button[1]/div[1]/span[2]").click()
        elif( RSI >= 70 and last_order=="sell" and EMA == "SELL"):
            print("Selling 1 stock of SONATSOFTW")
            last_order="sell"
            #sell 1 stock of SONATSOFTW 
            driver.find_element(By_XPATH, "//body[1]/div[2]/div[8]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]/div[2]/div[1]").click()
            time.sleep(2)
            driver.find_element(By_XPATH, "//button[1]/div[1]/span[2]").click()
        else:
